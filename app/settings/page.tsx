...
import { dailyTime, getDailyGoal } from "@/lib/db/queries";
...
export default async function SettingsPage() {
  ...
  const [goal, days] = await Promise.all([getDailyGoal(user.id), dailyTime(user.id, 190)]);
  ...
  return (
    <AppShell active="/settings" userName={user.name}>
      <SettingsView initialGoal={goal} data={days} todaySeconds={todaySeconds} />
    </AppShell>
  );
}
